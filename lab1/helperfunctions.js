...
function add(options) {
  ...
  if (options.checked != undefined) data.checked = options.checked;
...
function edit(data) {
  ...
  if (!isUpdated) console.log(`Element ${data.id} not Exist`);
}
...
editObject = (oldObj, newObj, index) => {
  if (newObj.title !== undefined) oldObj[index].title = newObj.title;

  if (newObj.checked !== undefined) oldObj[index].checked = newObj.checked;
...
function remove(data) {
  ...
  if (!isRemoved) console.log(`Element ${data.id} not Exist`);
}
